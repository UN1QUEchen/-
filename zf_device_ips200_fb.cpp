...
void ips200_show_color_image(uint16 x, uint16 y, const uint16 *image,
uint16 width, uint16 height)
{
    for(uint16 y_start = y; y_start < y + height; y_start++) {
        for(uint16 x_start = x; x_start < x + width; x_start++) {
